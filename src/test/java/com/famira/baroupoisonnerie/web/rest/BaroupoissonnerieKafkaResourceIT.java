...
import com.famira.baroupoisonnerie.config.KafkaSseConsumer;
import com.famira.baroupoisonnerie.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class BaroupoissonnerieKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/baroupoissonnerie-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/baroupoissonnerie-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/baroupoissonnerie-kafka/unregister"));
                return;
            }
        }
